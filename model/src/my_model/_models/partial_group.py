...
class PartialGroup(DomainModel):
    ...
    user_id: int = field(LogicalType.INTEGER, nullable=False, purpose="Identifies the user who owns the partial group.")
    # unique is not stated by the definition and resolves from the Model name-field default.
    name: str = field(LogicalType.STRING, nullable=False, unique=True, purpose="The partial group's display name.")
    status: bool = field(
        LogicalType.BOOLEAN, nullable=False, default=True, purpose="Indicates whether the partial group is active."
    )
    description: str | None = field(LogicalType.STRING, nullable=True, purpose="Describes the partial group.")
