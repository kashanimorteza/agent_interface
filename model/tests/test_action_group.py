...
def test_valid_construction_applies_declared_defaults():
    group = ActionGroup.model_validate(VALID)
    assert group.is_active is True


def test_rejects_unknown_field():
    assert_rejects_unknown_field(ActionGroup, VALID)


def test_requires_declared_non_nullable_fields():
    for field in ("user_id", "name"):
        assert_requires_field(ActionGroup, VALID, field)


def test_publishes_relationship_and_uniqueness_metadata():
    contract = ActionGroup.persistence_contract()
...
def test_serializes_and_generates_schema():
    assert_serializes_and_generates_schema(ActionGroup.model_validate(VALID))
