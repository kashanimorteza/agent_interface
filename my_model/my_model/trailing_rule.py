...
class TrailingRule(BaseModel):
    """An individual rule within a Trailing Group telling the system when and
    how to manage Take Profit and Stop Loss, through its activation condition
    and the parameters used to apply the required adjustments.
    """

    id: int = Field(description="The trailing rule's logical identity.")
    ...
    take_profit_adjustment: Decimal | None = Field(
        default=None, description="The take-profit adjustment applied when the rule is activated."
    )
    stop_loss_adjustment: Decimal | None = Field(
        default=None, description="The stop-loss adjustment applied when the rule is activated."
    )
