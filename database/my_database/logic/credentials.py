...
``hash`` is a one-way, salted, memory-hard transformation that can be verified
but never recovered. ``encrypted`` is a reversible transformation whose key is
read from the runtime environment and never from a package or generated file.
``plaintext`` stores the value as given.
"""
...
def generate_encryption_key() -> str:
    """A new key suitable for the encrypted at-rest mode."""
...
class CredentialTransformer:
    ...
    def _fernet(self) -> Fernet:
        if not self._key:
            raise ConfigurationError(
                f"encrypted at-rest mode needs the secret {self._key_name} in the runtime environment"
            )
        try:
        ...
        match mode:
            ...
            case "hash":
                ...
                digest = hashlib.scrypt(
                    value.encode("utf-8"), salt=salt, n=SCRYPT_N, r=SCRYPT_R, p=SCRYPT_P, maxmem=SCRYPT_MAXMEM
                )
                return "$".join(
                    (HASH_PREFIX, str(SCRYPT_N), str(SCRYPT_R), str(SCRYPT_P), _b64(salt), _b64(digest))
                )
            case "encrypted":
                return f"{ENCRYPTED_PREFIX}${self._fernet().encrypt(value.encode('utf-8')).decode('ascii')}"
    ...
    def verify(self, mode: str, stored: str, given: str) -> bool:
        match mode:
            ...
            case "hash":
                try:
                    ...
                    actual = hashlib.scrypt(
                        given.encode("utf-8"), salt=_unb64(salt), n=int(n), r=int(r), p=int(p), maxmem=SCRYPT_MAXMEM
                    )
            ...
            case "encrypted":
                try:
                    return hmac.compare_digest(self.recover(mode, stored).encode("utf-8"), given.encode("utf-8"))
    ...
    def recover(self, mode: str, stored: str) -> str:
        ...
        if prefix != ENCRYPTED_PREFIX or not token:
            raise InvalidOperation("stored value is not an encrypted credential")
        try:
            return self._fernet().decrypt(token.encode("ascii")).decode("utf-8")
        except InvalidToken as exc:
            raise InvalidOperation("the runtime encryption key does not recover this credential") from exc
