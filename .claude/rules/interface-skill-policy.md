...
- Except for the self-contained clear operation, a binding grants only the authority that the live Interface assigns to its generic operation. It never expands permissions, scope, contracts, or modes.
...
- Every explicit project value, generated contract, Policy, authority, permission, and write boundary is binding and is never silently overridden.
...
- Raise a question or blocker only when the missing or conflicting decision is critical: a reasonable choice could materially change project goals or scope, domain meaning, core architecture, security, data integrity, permissions, a contract, or an irreversible or destructive outcome; or no safe authorized path can continue.
- Agent discretion fills execution details inside authorized scope. It never invents project goals, product requirements, permissions, phases, contracts, or write authority, and it never widens the requested work.
...
- Technology guidance cannot change project requirements, phase scope, Interface Policy, contracts, write boundaries, workflow operations, or State authority.
