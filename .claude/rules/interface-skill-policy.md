...
- Stop only when no safe choice can be made without materially affecting project intent, core architecture, security, data integrity, permissions, a declared interface, or an irreversible action. Stopping means recording the reason where the Workflow keeps it — a Blocker for a condition that prevents continuation, an Open Question for a decision only the human can make — and then reporting it. A question asked only in conversation is lost the moment the session ends, and the next run has no way to know it was ever raised.
