...
description: Bring a completed implementation online through the Launch Process Component. Invoke only when the Human explicitly runs /my-interface-launch or when the declared coordinator /my-interface-implement invokes it through the Skill tool; never invoke it autonomously.
---
...
## Bridge
- Purpose: Bring a completed implementation online through the Launch Process Component.
- Owning Process Component: `.interface/implementation/process/launch/definition.md` and `.interface/implementation/process/launch/preferences.yaml`
...
- Inputs and outputs: exactly those the Launch Component Definition and Preferences declare.
- Authority: Follow the Launch Component authority and write only through the Process operation it defines.
...
2. Read the Launch Component Definition and Preferences listed above completely, then read every further authority they require. Do not rely on memory or prior summaries.
...
5. Execute the Launch Component as defined, without redefining its meaning or workflow. Repeating this Skill preserves valid work and never destructively replaces meaningful work.
