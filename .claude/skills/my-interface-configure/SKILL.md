...
description: Create the Interface Config files through the Configure Process Component. Invoke only when the Human explicitly runs /my-interface-configure or when the declared coordinator /my-interface-implement invokes it through the Skill tool; never invoke it autonomously.
---
...
## Bridge
- Purpose: Create the Interface Config files through the Configure Process Component.
- Owning Process Component: `.interface/implementation/process/configure/definition.md` and `.interface/implementation/process/configure/preferences.yaml`
...
- Trigger: Activate when invoked by the Human or a declared coordinator.
- Inputs and outputs: exactly those the Configure Component Definition and Preferences declare.
- Authority: Follow the Configure Component authority and write only through the Process operation it defines.
...
2. Read the Configure Component Definition and Preferences listed above completely, then read every further authority they require. Do not rely on memory or prior summaries.
...
4. Arguments (`$ARGUMENTS`): treat as the input the Component defines; if the Component defines no meaning for them, report that instead of guessing.
5. Execute the Configure Component as defined, without redefining its meaning or workflow. Repeating this Skill preserves valid work and never destructively replaces meaningful work.
