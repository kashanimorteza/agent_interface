...
description: Turn zero or more selected project phases into bounded Plans through the Plan Process Component. Invoke only when the Human explicitly runs /my-interface-plan or when the declared coordinator /my-interface-implement invokes it through the Skill tool; never invoke it autonomously.
...
## Bridge
- Purpose: Turn zero or more selected project phases into bounded Plans through the Plan Process Component.
- Owning Process Component: `.interface/implementation/process/plan/definition.md` and `.interface/implementation/process/plan/preferences.yaml`
...
- Trigger: Activate explicitly for zero or more phase selections, or when a declared coordinator requires a current Plan.
- Inputs and outputs: exactly those the Plan Component Definition and Preferences declare.
- Authority: Follow the Plan Component authority and write only through the Process operation it defines.
...
2. Read the Plan Component Definition and Preferences listed above completely, then read every further authority they require. Do not rely on memory or prior summaries.
...
5. Execute the Plan Component as defined, without redefining its meaning or workflow. Repeating this Skill preserves valid work and never destructively replaces meaningful work.
