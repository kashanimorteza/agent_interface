...
For every technical option the authorized work resolves from the current Implementation Preferences, resolve that option's declared `agent_skills` associations against the Skills currently discoverable and usable in this Runtime. Activate an associated Skill only when its option is actually selected and the Skill is currently available; use an optional Skill when available and continue without it when it is not, and stop the affected work when a required associated Skill is unavailable. Once activated, apply that Skill's complete applicable guidance to every implementation detail it governs — a result that contradicts activated guidance is not complete even when its other acceptance conditions pass. An associated Skill constrains how the option is used; it never expands scope or authority beyond the resolved Plan.
