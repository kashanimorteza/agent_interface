...
Establish Interface Understanding from `.interface/interface.md`, then read the complete Agent Module beginning with the Agent Module Guide at `.interface/agent/guide.md`, which carries the Agent Structure, and continuing through every current source that Structure names, discovering every declaration, resource, and explicit empty category dynamically on every invocation — never from a hardcoded list or one remembered by the Contract, this adapter, its helper, or an earlier run. Consume the Agent Preferences, portable Skill Contracts, optional prepared Markdown Skill files or directories matched by a declared Skill's exact stable key, native project artifacts, and runtime-reported activation state. A matched prepared file only identifies that Skill as Prepared rather than Constructed; an unmatched one is reported and never installed by inference. Treat Principles as mandatory contracts and Profiles as Human-owned desired state; never modify either. Use no Target Understanding and no other Interface Module.
