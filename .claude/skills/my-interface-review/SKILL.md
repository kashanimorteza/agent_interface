...
description: Review a phase Plan and its implemented result through the Review Process Component. Invoke only when the Human explicitly runs /my-interface-review or when the declared coordinator /my-interface-implement invokes it through the Skill tool; never invoke it autonomously.
...
## Bridge
- Purpose: Review a phase Plan and its implemented result through the Review Process Component.
- Owning Process Component: `.interface/implementation/process/review/definition.md` and `.interface/implementation/process/review/preferences.yaml`
...
- Trigger: Activate explicitly for zero or more phase selections after implementation exists.
- Inputs and outputs: exactly those the Review Component Definition and Preferences declare.
- Authority: Follow the Review Component authority and write only through the Process operation it defines.
...
2. Read the Review Component Definition and Preferences listed above completely, then read every further authority they require. Do not rely on memory or prior summaries.
...
5. Execute the Review Component as defined, without redefining its meaning or workflow. Repeating this Skill preserves valid work and never destructively replaces meaningful work.
