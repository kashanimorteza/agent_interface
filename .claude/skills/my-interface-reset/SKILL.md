...
description: Reconcile operational records and implementation outputs with one explicitly authorized reset scope through the Reset Process Component. Invoke only when the Human explicitly runs /my-interface-reset; never invoke it from another Skill, coordinator, or automatically.
...
## Bridge
- Purpose: Reconcile operational records and implementation outputs with one explicitly authorized reset scope through the Reset Process Component.
- Owning Process Component: `.interface/implementation/process/reset/definition.md` and `.interface/implementation/process/reset/preferences.yaml`
...
- Inputs and outputs: exactly those the Reset Component Definition and Preferences declare.
- Authority: Follow the Reset Component authority and write only through the Process operation it defines.
...
2. Read the Reset Component Definition and Preferences listed above completely, then read every further authority they require. Do not rely on memory or prior summaries.
...
4. Arguments (`$ARGUMENTS`): treat as the reset scope the Component defines; if the Component defines no meaning for them, report that instead of guessing.
5. Execute the Reset Component as defined, without redefining its meaning or workflow. Repeating this Skill preserves valid work and never destructively replaces meaningful work.
