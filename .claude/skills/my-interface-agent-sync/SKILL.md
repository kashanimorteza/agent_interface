...
- `self` — run the Realization procedure below on this adapter only — this file and the helper beside it — against the current `agent-sync` Contract. Reconcile nothing else, and never report the complete Module as synchronized from a `self` run.
