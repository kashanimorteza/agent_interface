...
For every technical option a selected phase resolves from the current Implementation Preferences, resolve that option's declared `agent_skills` associations against the Skills currently discoverable and usable in this Runtime. When an associated Skill is required and currently available, shape the governed Task's acceptance and verification to require observable conformance with that Skill's applicable guidance, not only completion of the underlying requirement. When a required associated Skill is unavailable, leave the affected phase incomplete and record the conflict; an optional Skill's absence never blocks planning.
