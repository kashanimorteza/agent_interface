...
Do not write, renumber, or re-scope tasks, and do not change `task_schema`, `task_states`, or a plan's `phase_titles`. Planning belongs to the `my_planner` Skill. If the plan is wrong, say so and stop.
