...
Configure prepares the operational Config records. Plan turns Target phases into bounded, understandable, and verifiable activities. Develop performs authorized implementation work. Review establishes whether selected-phase work satisfies its applicable authorities and stores detailed Findings in its History data. Implement coordinates the workflow, Launch activates a completed implementation, Reset reconciles an authorized scope, and State records aggregate operational position, progress, outcomes, and the History of every Skill, including Blockers and Open Questions. Each Skill records its execution metadata and concise report in State; detailed Skill-specific content belongs in its History item's `data`.
