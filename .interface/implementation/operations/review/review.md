...
Review owns its Findings and the History data that records what was reviewed. It does not own implementation, Plan, Target, or the active Workflow position, and it never enters or changes a Workflow Mode. It records aggregate Review progress and its History outcome under State.
...
Review works one phase at a time from current authorities. It reads the latest relevant Plan, Develop, and prior Review history records before judging. It establishes the Plan as its baseline, then, when available, examines the generated Source, Public Interface, implemented result, and evidence against that baseline. It writes detailed Findings and assurance results to its State History record through `data`, together with the common execution metadata.
...
- **Review ID** — the stable identifier of the current Review invocation, recorded in its State History data.
...
- **Consumes State** — uses current aggregate progress and prior Review History records without treating either as authority.
- **Consumed by Plan** — a recorded Gap identifies work the Plan must cover without copying the Finding into Plan content.
- **Consumed by State** — writes the phase's aggregate Review outcome and a History record containing execution metadata, a concise report, and Review-specific data including Findings.

<br>

Technical choices and defaults belong to Review Preferences, which currently define none. Review results are stored in State History; Review has no separate generated Config record.
...
**Rule:** Every Finding is stored in the Review Skill's State History `data`, and remains stored until an authorized Review establishes that it no longer holds or the human accepts it as it is. Its state is part of the History record.
...
**Boundary:** Review records its Findings and its own aggregate outcome in its State History item. It does not reopen Tasks, raise Blockers on another Component's behalf, or change the active Workflow mode to reflect what it found.
...
Review establishes fresh Interface Understanding and Target Understanding on every invocation. It reads the selected Plan, State, the latest relevant Planning and Develop History records, applicable Implementation authorities, synchronized Runtime rules, implementation, generated Source, public interfaces, prior Review History records, and evidence. It never reads Agent Module sources.
...
Review writes one State History record for every invocation. The record stores the start and completion times, measurable duration, available operation-level token usage, the Skills actually used with their available metrics and reports, a concise summary of the assurance outcome, and Review-specific `data`. That data includes the reviewed phase and Plan Revision, both assurance outcomes, Finding identifiers and their detailed evidence. Review has no separate generated Config record.
