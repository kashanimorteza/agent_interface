...
- **Agent Native Sync** — the Foundation instruction and Agent Skill `/my-interface-agent-native-sync` that reads the Agent Module and realizes it in the selected Agent Native.
- **Capability Realization Kind** — how a declared Agent capability becomes usable in the active Native: **Constructed**, realized from the Agent Module and owning Process Component; or **Installed**, supplied by an external provider and managed by the relevant Native mechanism.
