...
- For each declared Skill, resolve its Capability Realization Kind. A Skill with a matching prepared Markdown file or prepared directory under the Profile's file convention is Prepared: create the selected Runtime's required Skill folder and entrypoint, preserve the prepared content's semantic instruction meaning including every file of a prepared directory tree with its internal relative paths intact, and add or adapt only required native metadata. An Interface-owned Skill without one is Constructed from its portable Contract. A declared external Skill is Installed: observe its actual state, classify it as no change when already present and usable, install it additively through the native mechanism named by its owning Agent Extension or Agent Integration declaration when absent, and report it as activation required or blocked when that mechanism is unavailable or demands credentials, external trust, or broader scope. Never build or transfer content for it. Report an unmatched prepared file or directory instead of inferring a Skill from it.
