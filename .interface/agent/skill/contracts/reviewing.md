...
- Before beginning any assurance for a phase, verify that implementation and generated Source exist. If they do not, stop Review for that phase and report that Developing or Implement must create the implementation first.
...
- If Config is stale or invalid for the current phase, record a Finding naming Configure as its owning operation and withhold both assurances until Configure has resolved it. If the phase's technical Environment is insufficient, record a Finding naming Developing.
- If current authorities declare changed public metadata or Config structure, record a Finding naming Configure; if they declare changed Component paths, packages, versions, tools, or connections, record a Finding naming Developing; if they declare changed Platform requirements, record a Finding naming Launch. Do not assess the phase until the owning operation has resolved it.
