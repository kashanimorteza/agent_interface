...
- If Config or Environment readiness is stale or insufficient for the current phase, record a Finding naming Configure as its owning operation and withhold both assurances until Configure has resolved it.
- If current authorities declare changed Component paths, packages, versions, tools, public metadata, connections, or Platform requirements, record a Finding naming Configure as its owning operation and do not assess the phase until Configure has resolved it.
