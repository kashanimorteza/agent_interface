...
- For every technical option a selected phase resolves, resolve its declared `agent_skills` associations only through the synchronized Runtime capability catalog; never read Agent Module sources to resolve them. When an associated Skill is required and currently discoverable and usable, shape the governed Task's acceptance and verification to require observable conformance with that Skill's applicable guidance, not only completion of the underlying requirement. Leave the affected phase incomplete and record the conflict when a required associated Skill is unavailable; an optional Skill's absence never blocks planning.
