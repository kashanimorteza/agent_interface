...
Read an authorized located resource before relying on it. Memory, operational records, conversation history, and prior summaries never substitute for current authoritative sources; after a session resume or context compaction, re-establish the Understanding the active role requires before mutating anything. Agent Module changes are deliberately excluded and have no Runtime effect until the Human explicitly runs Agent Native Sync in mode 1 (sync self), then mode 2 (sync component). Its install mode, mode 3, reads the Module only to resolve capabilities to provision; it never repairs Runtime drift and never serves as a second synchronization path.
...
- Only the explicitly invoked Agent Native Sync in its sync modes may compare Runtime artifacts with Agent Module authorities and repair drift. Its install mode may read the Module only to resolve which capabilities to provision; it never repairs drift or reconciles Rules, Skills, or other Runtime artifacts against Module authorities. Every other Skill reports a missing or unusable Runtime capability; none of them inspect the Agent Module or invoke Agent Native Sync automatically.
