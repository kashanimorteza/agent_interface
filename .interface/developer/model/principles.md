...
The concrete syntax for the canonical module-qualified import is defined by Model Preferences and demonstrated in the Component `README.md`. The logical requirement is that public Model modules and types are imported explicitly and addressed through the package namespace.
...
Public modules and Model types MAY be re-exported from the package root when the selected implementation supports it. The exact re-export syntax belongs to Model Preferences and the Component `README.md`.
